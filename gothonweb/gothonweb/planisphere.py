class Room(object):
    ...
    def go(self, direction):
        return self.paths.get(direction, None) # returns none by default if no key

    def add_paths(self, paths):
        self.paths.update(paths)

    def get_choices(self):
        choices = list(self.paths.keys())
        return choices
...
them could be damaged but you don't have time to look.  There are 5 pods,
...
def load_room(name):
    ...
    # input room name, get the room object
...
def name_room(room):
    ...
    for key, value in globals().items():
        if value == room: # Value here is the room object
            return key
    # input room object, get room name.

# print(globals().items())

# https://www.programiz.com/python-programming/methods/built-in/globals
# The global scope contains all functions, 
# variables which are not associated to any class or function.

# The globals table dictionary is the dictionary of the current module 
# (inside a function, this is a module where it is defined, not the 
